...
library(bsicons)
library(bslib)
 # add the icons then can publish
...
fluidRow(
  ...
  column(
    ...
    value_box(title = "Total WATCH-DM Points",value = textOutput("sum"), showcase = bs_icon("bar-chart")),
    value_box(title = "5-year HFH risk: Original WATCH-DM Score", value = textOutput("orig"), showcase = bs_icon("graph-up")),
  ...
  column(
    ...
    value_box(title = "The SDI Quintile according to the zip code",value = textOutput("q2"), showcase = bs_icon("pie-chart")),
    value_box(title = "5-year recalibrated HFH risk: sdiWATCH-DM Score", value = textOutput("recab2"),
  ...
  column(width = 6,
         h6("Developed by Salil Deo (svd14@case.edu), Ayush Patel and Sadeer Al-Kindi")
         )
